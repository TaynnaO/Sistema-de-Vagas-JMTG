...
$sql = "SELECT c.id as id, c.foto as foto, c.nome as n1, COALESCE(p.nome, 'Função não cadastrada') as n2 FROM `admin` c
left JOIN `funcao` p
ON c.id_funcao = p.id ";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<div class='container-vagas'>"."<div class='vagas'>"."<img src='uploads/$row[foto]' style='width:50px; height:50px; border-radius:100%;'>"." Nome: " . $row["n1"]."<br>" . " Função: " . $row["n2"]. "<br>"."</div>"."<div class='btn-baixar candidatar' data-id='$idcurriculo' style='cursor:pointer;'>"."<p>Baixar currículo</p>"."</div>"."</div>";
