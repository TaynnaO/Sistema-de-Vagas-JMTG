...
$sql = "SELECT * FROM `admin` WHERE id = $id_do_usuario and foto is not null";
...
echo "<header style='background:white; margin-top:-10px; padding:5px;'>
    <ul>
        <a href='../authenticated/homeAdmin.php'> <li>
  ...
  <a href='perfilAdmin.php'><li>Editar perfil</li></a>
  <a href='../authenticated/funcao.php'> <li>Função</li></a>
  ...
  <a href='./logout.php'><li>Sair</li></a>
    ...
    <li class='dropdown-btn'>Perfil Admin</li>
    <svg xmlns='http://www.w3.org/2000/svg' style='width:10px; color:green;' viewBox='0 0 320 512'><!--! Font Awesome Pro 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d='M137.4 374.6c12.5 12.5 32.8 12.5 45.3 0l128-128c9.2-9.2 11.9-22.9 6.9-34.9s-16.6-19.8-29.6-19.8L32 192c-12.9 0-24.6 7.8-29.6 19.8s-2.2 25.7 6.9 34.9l128 128z'/ ></svg>
    </div>
    <ul class='dropdown-menu'>
    <a href='perfilAdmin.php'><li>Editar perfil</li></a>
...
$sql = "SELECT * FROM `admin` WHERE id = $id_do_usuario";
$result = $conn->query($sql);
    ...
    while($row = $result->fetch_assoc()) {
        echo "<form method='POST' action='' enctype='multipart/form-data'>
            ...
            <div class='container-inputs'>
                ...
                <input type='date' value='{$row['dtNascimento']}' name='dtNascimento'>
            </div>
...
if (isset($_POST['atualizar_cadastro'])) {
    ...
    if (isset($_FILES["foto"]) && $_FILES["foto"]["error"] === UPLOAD_ERR_OK) {
        ...
        $sql = "SELECT foto FROM admin WHERE id=$id_do_usuario";
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();
        $foto_atual = $row["foto"];

        // Define o novo nome da imagem no banco de dados
        $sql = "UPDATE admin SET foto ='$new_file_name', nome='$nome', email='$email', cpf='$cpf', rg='$rg', celular='$celular', dtNascimento='$dtNascimento' WHERE id=$id_do_usuario";
        ...
        $sql = "UPDATE admin SET nome='$nome', email='$email', cpf='$cpf', rg='$rg', celular='$celular', dtNascimento='$dtNascimento' WHERE id=$id_do_usuario";
        if ($conn->query($sql) === TRUE) {
            echo "Cadastro atualizado com sucesso!";
